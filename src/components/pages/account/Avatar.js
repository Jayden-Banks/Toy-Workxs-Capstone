  ...
  const handleClick = async () => {
    try {
      const res = await axios.get(
        `https://pokeapi.co/api/v2/pokemon/${currSearch}`
      );
  ...
  const showPokemon = () => {
    return (
      <div className="div-individual-product " id="div-individual-pokemon">
        ...
        ></div>
        <h4 className="h4-product-title h4-product-title-space">
          {searchedPokemon}
        </h4>
        <button className="button-product-add" onClick={() => setIcon()}>
          Set Icon
        </button>
      </div>
    );
