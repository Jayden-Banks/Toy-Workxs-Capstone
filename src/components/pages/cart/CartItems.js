...
function CartItems({ notificationSetter, setTotalPrice, totalPrice }) {
  ...
  const handleQuantityClick = async (e, productId, quantity) => {
    ...
    try {
      ...
      console.log(err);
      notificationSetter(
        <h3 className="h3-error-submit">Failed to update Cart</h3>
      );
      setTimeout(function () {
        notificationSetter("");
      }, 1500);
    ...
    const arrDisplay = dataCopy.map((element, index) => {
      ...
      return (
        <div className="div-single-cart-item" key={index}>
          <div
            className="div-image-cart-item div-product-image"
          ...
          <div className="div-text-cart-item">
            ...
            <div className="div-buttons-cart-item">
              <input
                ...
                onClick={(e) =>
                  handleQuantityClick(e.target.value, productId, quantity)
                }
                disabled={quantity <= 1 ? "disabled" : false}
                style={quantity <= 1 ? { filter: "invert(100%)" } : {}}
                value="minus"
                className="input-cart-item"
                width="20px"
              />
              <button
                className="button-remove-cart-item button-cart"
                onClick={() => handeleRemoveClick(productId)}
              >
                Remove
              </button>
              <input
